...
class RectShape:
    ...
    @fill.setter
    def fill(self, v):
        self._fill = get_rgba_tuple(v, alpha=self.alpha)
...
class Box(RectShape):
    ...
    def draw(self, draw: ImageDraw):
        ...
        if hasattr(self, 'depth') and self.depth > 0:
            ...
            draw.line([self.x1 + self.depth, self.y1 - self.depth, self.x1 + self.depth, self.y2 - self.depth], pen)
            draw.line([self.x1 + self.depth, self.y2 - self.depth, self.x1, self.y2], pen)
            draw.line([self.x1 + self.depth, self.y2 - self.depth, self.x2 + self.depth, self.y2 - self.depth], pen)

            draw.polygon([self.x1, self.y1,
                          self.x1 + self.depth, self.y1 - self.depth,
                          self.x2 + self.depth, self.y1 - self.depth,
                          self.x2, self.y1
                          ], pen, brush_s1)

            draw.polygon([self.x2 + self.depth, self.y1 - self.depth,
                          self.x2, self.y1,
                          self.x2, self.y2,
                          self.x2 + self.depth, self.y2 - self.depth
                          ], pen, brush_s2)
        ...
        if hasattr(self, 'text'):
            ...
            bbox_size = bbox[2] - bbox[0], bbox[3] - bbox[1]  # Calculate the size of the bounding box
            img_text = Image.new('RGBA', bbox_size, (0, 0, 0, 0))
            draw_text = ImageDraw.Draw(img_text)
            draw_text.text((0, 0), self.text, fill=self.outline, font=font)  # Draw the text at (0, 0) position
            draw.bitmap((self.x1, self.y1), img_text, fill=self.outline)  # Paste the text image onto the main image
...
def get_rgba_tuple(color: Any, alpha: float = 1) -> tuple:
    """
    ...
    if len(rgba) == 3:
        rgba = (rgba[0], rgba[1], rgba[2], int(alpha * 255))
    return rgba
